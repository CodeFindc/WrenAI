---
name: offline-wren-generate-mdl
description: "Generate a Wren MDL project offline in a LAN environment by exploring a database with available local tools (SQLAlchemy, local database drivers, or raw SQL). Guides agents through schema discovery, type normalization, and MDL YAML generation using the local wren CLI. Designed for air-gapped/offline networks."
license: Apache-2.0
metadata:
  author: wrenai
  version: "2.3-offline"
...
# Generate Wren MDL (Offline) — CLI Agent Workflow

> [!NOTE]
> **Offline Environment Guide**
> This skill is specifically designed for fully offline / local area network (LAN) / air-gapped environments.
> Do NOT attempt to connect to external URLs (such as fetching GitHub version indexes or downloading external Python packages). All packages must be installed locally (e.g., using `pip install -e .` or local offline wheels).
...
are available in its local environment for introspection; the local wren CLI handles
...
- **wren CLI installed locally.** The agent MUST verify the installation first by running `wren version` or `wren --help`. If the command is available, proceed normally. If it is NOT found, the agent MUST NOT attempt to run `pip install` or download packages; instead, it must stop and remind the user to install it.
- A working connection to a local or LAN database (credentials available to the agent).
- A wren profile configured locally (`wren profile add`) or connection info ready.
...
1. Verify connectivity to the local/LAN database using whichever local tool is available:
   - If SQLAlchemy is installed: `engine.connect()` test
   - If database driver is installed: simple query like `SELECT 1`
...
2. Ask the user:
   ...
   - The **datasource type** for wren (e.g., `postgres`, `mysql`, `clickhouse`, `duckdb`) — needed for type normalization dialect
...
Use whatever introspection method is locally available. Here are common approaches
...
### Option B: Local database-specific driver

- **psycopg / asyncpg (Postgres):** Query `information_schema.columns` and `information_schema.table_constraints`
- **mysqlclient / pymysql:** Query `information_schema.columns` and key constraints
- **duckdb:** Query `information_schema.columns`
...
> hand-write `GROUP BY` / `DATE_TRUNC` SQL.
